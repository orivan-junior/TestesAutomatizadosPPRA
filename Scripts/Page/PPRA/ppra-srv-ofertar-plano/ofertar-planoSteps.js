...
const getApiHeadersOfertarPlano = () => {
    ...
    const caKey = process.env.CA_KEY_OFERTAR_PLANO;
    if (!caKey) {
...
async function callOfertarPlanoApi(request, payload) {
    ...
    const endpoint = "/comercializacao/lista-oferta";
...
Given('que eu esteja autenticado para ofertar plano', async () => {
    isAuthenticated = true;
    console.log('Autenticação para o cenário ativada. A CA_KEY do .env será utilizada.');
});

Given('que eu utilize o payload do arquivo ofertar plano {string}', async ({}, filePath) => {
...
When('eu envio uma requisição POST para ofertar plano', async function({ request }) {
 ...
 */
When('eu envio uma requisição GET para o endpoint para ofertar plano {string}', async function({ request }, endpoint) {
...
Then('a resposta de uma oferta de plano deve ser válida', async function() {
