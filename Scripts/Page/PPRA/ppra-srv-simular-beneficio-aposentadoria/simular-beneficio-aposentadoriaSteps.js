...
async function callSimularBeneficioApi(request, payload) {
    const baseUrl = process.env.BASE_URL_SIMULAR_BENEFICIO_APOSENTADORIA;
    if (!baseUrl) {
        throw new Error("ERRO: A variável de ambiente BASE_URL_SIMULAR_BENEFICIO_APOSENTADORIA não está configurada no .env.");
    }
    const endpoint = "/simulador/send";
    ...
    return await request.post(fullUrl, {
        headers: getApiHeadersSimularBeneficio(),
        data: payload,
...
Given('que eu esteja autenticado', async () => {
    isAuthenticated = true;
    console.log('Autenticação para o cenário ativada. A CA_KEY do .env será utilizada.');
});

Given('que eu utilize o payload do arquivo {string}', async ({}, filePath) => {
...
When('eu envio uma requisição POST para simular o benefício de aposentadoria', async function({ request }) {
    ...
    this.apiResponse = await callSimularBeneficioApi(request, requestPayload);
 ...
 */
When('eu envio uma requisição GET para o endpoint {string}', async function({ request }, endpoint) {
